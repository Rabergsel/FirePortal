window.FirePortal =
    {
        ...
        loading: {
            ...
            loadMonaco: function () {
                ...
                monaco.editor.defineTheme('FirePortal-theme', {
                    base: 'vs-dark',
                    inherit: true,
                    rules: [],
                    colors: {
            ...
            checkConnection: async function (url, threshold) {
                ...
                try {
                    ...
                    if (latency > threshold) {
                        FirePortal.toasts.warning(`High latency detected: ${latency}ms. FirePortal might feel laggy. Please check your internet connection`);
                    }
                } catch (error) {
                }
            }
        ...
        flashbang: {
            run: function () {
                ...
                light.onanimationend = FirePortal.flashbang.clean;
            },
            clean: function () {
                const light = document.getElementById("flashbang");
        ...
        keyListener: {
            register: function (dotNetObjRef) {
                FirePortal.keyListener.listener = (event) => {
                    // filter here what key events should be sent to FirePortal
                ...
                window.addEventListener('keydown', FirePortal.keyListener.listener);
            },
            unregister: function (dotNetObjRef) {
                window.removeEventListener('keydown', FirePortal.keyListener.listener);
            }
        },
        serverList: {
            init: function () {
                if (FirePortal.serverList.Swappable) {
                    FirePortal.serverList.Swappable.destroy();
                }

                let containers = document.querySelectorAll(".draggable-zone");

                if (containers.length !== 0) {
                    FirePortal.serverList.Swappable = new Draggable.Sortable(containers, {
                        draggable: ".draggable",

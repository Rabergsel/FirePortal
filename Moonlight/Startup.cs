...
using FirePortal.App.Services.Mail;
using FirePortal.App.Services.Notifications;
using FirePortal.App.Services.Plugins;
using FirePortal.App.Services.Sessions;
using FirePortal.App.Services.Statistics;
using FirePortal.App.Services.Tickets;
using Sentry;
using Serilog;
using Serilog.Events;
using Stripe;
using SubscriptionService = FirePortal.App.Services.SubscriptionService;
...
public class Startup
{
    ...
    {
        Logger.Info("Starting FirePortal panel");
        ...
        if (!Uri.TryCreate(ConfigService.Get().FirePortal.AppUrl, UriKind.RelativeOrAbsolute, out Uri uri))
        {
            Logger.Fatal("Invalid app url");
            return;
        }
        
        if(ConfigService.DebugMode || uri.HostNameType == UriHostNameType.IPv4 || !ConfigService.Get().FirePortal.LetsEncrypt.Enable)
            await WebApplication.RunAsync();
        else
            await WebApplication.RunAsync(ConfigService.Get().FirePortal.AppUrl);
    }
    ...
    private Task StartBackgroundServices()
    {
        ...
        _ = WebApplication.Services.GetRequiredService<FirePortalService>();
    ...
    private Task BuildSingletonServices()
    {
        ...
        WebApplicationBuilder.Services.AddSingleton<FirePortalService>();

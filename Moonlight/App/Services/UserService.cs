...
using FirePortal.App.Exceptions;
...
public class UserService
{
    ...
    private readonly FirePortalService FirePortalService;
    ...
    public UserService(
        ...
        FirePortalService FirePortalService)
    {
        UserRepository = userRepository;
        TotpService = totpService;
        ConfigService = configService;
    ...
    public async Task<string> Register(string email, string password, string firstname, string lastname)
    {
        ...
        if (ConfigService.Get().FirePortal.Auth.BlockLinuxUsers && IdentityService.Device.Contains("Linux"))
            throw new DisplayException("This operation was disabled");

        if (ConfigService.Get().FirePortal.Auth.CheckForBots)
        {
        ...
        {
            if ((DateTime.UtcNow - FirePortalService.StartTimestamp).TotalMinutes < 15)
                admin = true;
            else
                throw new DisplayException("You have to register within 15 minutes after the start of FirePortal to get admin permissions. Please restart FirePortal in order to register as admin. Please note that this will only works once and will be deactivated after a admin has registered");
        }
    ...
    {
        if (ConfigService.Get().FirePortal.Auth.DenyLogin)
            throw new DisplayException("This operation was disabled");

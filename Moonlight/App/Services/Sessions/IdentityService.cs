...
public class IdentityService
{
    ...
    private Task LoadDevice()
    {
        ...
        try
        {
            ...
            if (userAgent.Contains("FirePortal.App"))
            {
                var version = userAgent.Remove(0, "FirePortal.App/".Length).Split(' ').FirstOrDefault();

                Device = "FirePortal App " + version;
                return Task.CompletedTask;
            }
    ...
    public async Task<bool> GetBotStatus()
    {
        var webDriverStatus = await JsRuntime
            .InvokeAsync<bool>("FirePortal.utils.getWebDriverStatus");

        if (webDriverStatus)
            return true;

        var languagesStatus = await JsRuntime
            .InvokeAsync<bool>("FirePortal.utils.getLanguagesStatus");

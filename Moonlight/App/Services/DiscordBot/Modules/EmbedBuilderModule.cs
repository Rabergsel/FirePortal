...
public class EmbedBuilderModule : BaseModule
{
    ...
    private EmbedAuthorBuilder AThing(IUser user)
    {
        ...
        if (randomNumbers.Contains(random.Next(1, 24)))
            return new EmbedAuthorBuilder().WithName(Client.CurrentUser.Username + " - The Rick version").WithUrl(ConfigService.Get().FirePortal.AppUrl).WithIconUrl("https://cdn.discordapp.com/attachments/750696464014901268/1092783310129860618/rick.gif");
        
        return new EmbedAuthorBuilder().WithName(Client.CurrentUser.Username).WithUrl(ConfigService.Get().FirePortal.AppUrl).WithIconUrl(Client.CurrentUser.GetAvatarUrl());
    }
}

...
public class LetsEncryptService
{
    ...
    public async Task AutoProcess()
    {
       try
       {
             if (!ConfigService.Get().FirePortal.LetsEncrypt.Enable)
            return;
    ...
    private Task LoadCertificate()
    {
        try
        {
            Certificate = new X509Certificate2(
                LetsEncryptCertPath, 
                ConfigService.Get().FirePortal.Security.Token
            );
    ...
    private async Task Renew()
    {
        ...
        var uri = new Uri(ConfigService.Get().FirePortal.AppUrl);
        var config = ConfigService.Get().FirePortal.LetsEncrypt;
    ...
    private Task<bool> CheckNeedsRenewal()
    {
        ...
        var existingCert = new X509Certificate2(LetsEncryptCertPath, ConfigService.Get().FirePortal.Security.Token);
